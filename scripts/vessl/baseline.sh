#!/bin/bash
# VESSL Run: Baseline MTP (Unified)
#
# 사용법:
#   ./scripts/vessl/baseline.sh --ngpus 4
#   ./scripts/vessl/baseline.sh --ngpus 1 --batch-size 8 --grad-accum 2
...
# Train command 생성
if [ "$NGPUS" -eq 1 ]; then
  TRAIN_CMD="uv run python -m weighted_mtp.pipelines.run_baseline --config $CONFIG$OVERRIDE_ARGS"
  NCCL_ENV_VARS=""
else
...
# VESSL Run 실행
echo ""
echo "=== VESSL Run 실행: Baseline MTP ($NGPUS-GPU) ==="
