#!/bin/bash
# VESSL Run: Verifiable WMTP (A100 4-GPU)
# 주의: Critic checkpoint가 필요합니다
...
# Critic checkpoint 경로 설정 (사용자 입력 또는 기본값)
echo ""
read -p "Critic checkpoint 경로를 입력하세요 (기본값: storage/checkpoints/critic/critic-pretrain/checkpoint_best.pt): " CRITIC_CHECKPOINT_INPUT
CRITIC_CHECKPOINT="${CRITIC_CHECKPOINT_INPUT:-storage/checkpoints/critic/critic-pretrain/checkpoint_best.pt}"
echo "사용할 checkpoint: $CRITIC_CHECKPOINT"

# 임시 YAML 파일 생성 (환경변수 치환)
TEMP_YAML=$(mktemp)
cp scripts/vessl/verifiable_4gpu.yaml "$TEMP_YAML"
...
# VESSL Run 실행
echo ""
echo "=== VESSL Run 실행: Verifiable WMTP (4-GPU) ==="
...
echo ""
echo "참고: Critic checkpoint를 사용하여 TD error weighting을 수행합니다."
