...
class TestComputeMTPSelectiveWeights:
    ...
    def test_basic_shape(self):
        ...
        assert "head_0_ratio" in stats

    def test_head_0_always_selected(self):
        """Head 0 (t+1)은 무조건 선택"""
        batch_size = 2
        seq_len = 10
        n_future = 4
        ...
        # Head 0은 attention_mask와 동일해야 함
        assert torch.allclose(weights[:, :, 0], attention_mask.float())
        assert stats["head_0_ratio"] == 1.0

    def test_padding_handling(self):
        """Padding 토큰 (attention_mask=0) 제외 검증"""
        batch_size = 2
        seq_len = 10
        n_future = 4
        vocab_size = 100
    ...
    def test_top_k_selection(self):
        ...
        # Head 1 선택 비율이 k_percent 근처인지 확인
        # k_percent=0.5이므로 약 50%가 선택되어야 함
        head_1_ratio = stats.get("head_1_ratio", 0)
        assert 0.3 < head_1_ratio < 0.7, f"Head 1 ratio {head_1_ratio} not in expected range"
...
class TestComputeRho1Stats:
    ...
    def test_basic_stats(self):
        ...
        stats = compute_rho1_stats(weights, attention_mask)
