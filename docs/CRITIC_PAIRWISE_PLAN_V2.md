# Critic 일반화 성능 확보 계획서 V2

Verifiable Critic의 Validation 성능 붕괴(Shortcut Learning) 현상을 해결하기 위한 최종 수정 계획.
...
| 데이터 | Pointwise 샘플 | Pairwise (Correct, Incorrect) |
| 평가 지표 | F1, Pred Gap | Pairwise Accuracy |
| 샘플링 방식 | difficulty, ~~problems~~ | difficulty, pairwise |

### 2.3 Stage 2 Auxiliary Loss 문제 해결

**문제**: Stage 2에서 Backbone이 학습되면 Embedding 분포가 변화하여 Critic Drift 발생.
그러나 SFT는 Correct 데이터만 사용하므로 Pairwise Ranking Loss 계산 불가.

**해결 (Triplet SFT)**:
```
Stage 2 데이터 구조:
┌─────────────────────────────────────────────────┐
│ Batch: (Instruction, Correct, Incorrect)        │
├─────────────────────────────────────────────────┤
│ Policy Loss: CE(Correct) only                   │
│ Critic Loss: Ranking(V_correct, V_incorrect)    │
└─────────────────────────────────────────────────┘
```

- Forward: Correct와 Incorrect 모두 통과
- Policy Loss: Correct 토큰에 대해서만 CE Loss (Incorrect는 마스킹)
- Critic Loss: Correct와 Incorrect의 Value 비교 (Ranking Loss)
- Trade-off: 속도 ~0.5x (연구 무결성을 위해 감수)
...
| `data/dataloader.py` | 수정 | Collator 분기 로직 추가 |
| `pipelines/run_critic.py` | 수정 | Pairwise ranking loss 지원 |
...
| 파일 | 삭제 내용 |
|------|----------|
| `data/datasets.py` | `_sample_by_problem_id()` 함수 전체 (약 110줄) |
| `data/datasets.py` | `_compute_sampling_indices_from_metadata()`에서 `problems` 분기 |
| `data/dataloader.py` | docstring에서 problems 관련 설명 |
| `pipelines/run_critic.py` | `sampling_method == "problems"` 관련 분기 |
...
[전체 Pairs: 50,000개]
    │
    ├── Rank 0: pairs[0::4] → 12,500개 쌍
    ├── Rank 1: pairs[1::4] → 12,500개 쌍
    ├── Rank 2: pairs[2::4] → 12,500개 쌍
    └── Rank 3: pairs[3::4] → 12,500개 쌍

각 GPU에서:
┌─────────────────────────────────────┐
│ Batch (local)                       │
│ pos_input_ids[0] ←→ neg_input_ids[0]│  쌍 유지 ✓
│ pos_input_ids[1] ←→ neg_input_ids[1]│  쌍 유지 ✓
│ ...                                 │
└─────────────────────────────────────┘
```
...
| 구분 | 상태 | 설명 |
|------|------|------|
| 데이터 분배 | ✓ | 쌍 단위로 rank에 할당 (`pairs[rank::world_size]`) |
| Collator | ✓ | 로컬 GPU에서 pos/neg 함께 구성 |
| Forward | ✓ | 같은 GPU에서 pos, neg 모두 처리 |
| Loss 계산 | ✓ | 로컬에서 ranking loss 계산 |
| Gradient | ✓ | FSDP가 all-reduce 처리 |
| drop_last | ✓ | 기존 코드에서 `drop_last=(world_size > 1)` 설정됨 |

### 4.3 메모리 고려사항

```python
batch_size = 32  # 쌍 단위
# → pos 32개 + neg 32개 = 64 시퀀스 forward
# → 기존 대비 메모리 2배 필요
...
### 5.2 collators.py - PairwiseDataCollator 추가
...
def _compute_sampling_indices_from_metadata(...):
    ...
    # 3. problems 방식 삭제됨
        ...
        raise ValueError(f"잘못된 sampling_method: {sampling_method}. 'difficulty' 또는 'pairwise' 사용")
```
...
def _read_jsonl_pairwise(
    ...
    # 인덱스별 샘플 로드 (기존 _read_jsonl_by_indices 로직 활용)
    idx_to_sample = _load_samples_by_indices(jsonl_path, all_indices)
...
def create_dataloader(
    ...
    Args:
        sampling_config: 샘플링 설정 딕셔너리
            - sampling_method: "difficulty" 또는 "pairwise"
            - difficulty: {n_samples, auto_data_balancing, correct_ratio, ...}
            - pairwise: {n_pairs}
    """
    # ... 기존 데이터셋 로드 로직 ...
...
### 5.5 run_critic.py - Pairwise Loss 지원
...
```python
# Pairwise 학습
pos_outputs = adapter(batch["pos_input_ids"], batch["pos_attention_mask"], return_value_logits=True)
neg_outputs = adapter(batch["neg_input_ids"], batch["neg_attention_mask"], return_value_logits=True)
...
# Pairwise accuracy 로깅
pairwise_acc = compute_pairwise_accuracy(...)
```

### 5.6 Stage 2 (Verifiable Training) - Triplet SFT

**run_verifiable.py 수정**:

```python
# Stage 2: Policy + Auxiliary Critic 학습
# 데이터: Triplet (Instruction, Correct, Incorrect)
...
# 1. Policy Loss: Correct에 대해서만 CE Loss
policy_loss = compute_mtp_loss(
    logits=pos_outputs["logits"],
    labels=batch["pos_labels"],
    weights=weights,  # TD error 기반 가중치
)

# 2. Auxiliary Critic Loss: Ranking Loss (Critic Drift 방지)
critic_loss = pairwise_ranking_loss(
    v_pos=pos_outputs["value_logits"],
    v_neg=neg_outputs["value_logits"],
    mask_pos=(batch["pos_labels"] != -100).float(),
    mask_neg=(batch["neg_labels"] != -100).float(),
...
# 3. Total Loss
total_loss = policy_loss + value_coef * critic_loss  # value_coef = 0.5
```

### 5.7 critic_pairwise.yaml - Config
...
training:
  n_epochs: 1.0
  batch_size: 16  # 쌍 단위 (실제 32 시퀀스 forward)
...
1. **Step 1**: `datasets.py`에서 `_sample_by_problem_id()` 함수 삭제
2. **Step 2**: `datasets.py`에서 `_compute_sampling_indices_from_metadata()` problems 분기 삭제
3. **Step 3**: `run_critic.py`에서 problems 관련 분기 삭제
4. **Step 4**: `dataloader.py` docstring에서 problems 설명 삭제

### Phase 2: 구현 (추가)

5. **Step 5**: `value_head.py` 수정 - dropout 인자 추가
6. **Step 6**: `collators.py`에 `PairwiseDataCollator` 추가
7. **Step 7**: `datasets.py`에 `_sample_pairwise()`, `_read_jsonl_pairwise()` 추가
8. **Step 8**: `dataloader.py`에 Collator 분기 로직 추가
9. **Step 9**: `run_critic.py`에 pairwise loss 및 메트릭 추가
10. **Step 10**: `critic_pairwise.yaml` config 생성

### Phase 3: 검증

11. **Step 11**: 로컬 단일 GPU 테스트
12. **Step 12**: FSDP 4-GPU 분산 학습 테스트
...
| Train-Val Accuracy Gap | 큼 (0.66 vs 0.56) | **<5%p 차이** |
...
- **Triplet SFT**: Stage 2에서도 Critic의 비교 능력 유지
...
- 이전 버전: `docs/CRITIC_PAIRWISE_PLAN.md`
